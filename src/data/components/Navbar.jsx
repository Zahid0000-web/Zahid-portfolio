...
const Navbar = ({ terminalMode, setTerminalMode }) => {
  ...
  const handleTerminalToggle = () => setTerminalMode(!terminalMode);
      ...
      <div className="max-w-7xl mx-auto px-3 py-2.5 flex items-center justify-between">
        ...
        >
          ...
          <TypeAnimation
            ...
            sequence={
              terminalMode
                ? ["$ whoami", 2000, "visitor[MZ]", 2000]
                : ["[MZ]", 2000, "Web Developer", 2000]
            }
            wrapper="span"
            speed={50}
            repeat={Infinity}
        ...
        <div className="flex p-2 border border-gray-400 dark:border-gray-600 rounded-full items-center gap-4">
          <a href="https://github.com/Zahid0000-web" target="_blank" title="Source Code">
            <Github size={20} />
          </a>
          {!terminalMode && (
            <button onClick={handleThemeToggle} title="Toggle Theme" className="transition-transform duration-300 relative z-10">
              {darkMode ? <Sun size={20} /> : <Moon size={20} />}
            </button>
          )}

          <button onClick={handleTerminalToggle} title="Toggle Terminal Mode" className="transition-transform duration-300 relative z-10">
            <Terminal size={20} />
          </button>
        </div>
      </div>
